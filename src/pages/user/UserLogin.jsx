...
const UserLogin = ({ onBack, onLoginSuccess, onGoToRegister }) => {
    ...
    const handleLogin = async (e) => {
        ...
        try {
            ...
            // Sadece 'user' veya 'admin' rolündeyse giriş yapsın
            if (data.role === 'user' || data.role === 'admin' || data.role === 'beneficiary') {
                localStorage.setItem("user", JSON.stringify(data));
                onLoginSuccess({ name: data.username || data.name, role: 'user', id: data.id, balance: data.balance });
            } else {
                setError('Bu hesap İhtiyaç Sahibi rolünde değil.');
                setLoading(false);
            }
        } catch (err) {
